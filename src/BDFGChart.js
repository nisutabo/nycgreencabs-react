    ...
    const handleMiniChartClick = (e,i) => {
        if (i[0]){
          ...
          this.state.hours.forEach((hour) => {
            totalRidesPerHour.push(monthHours[month][day][hour]['totalrides'])
            totalFaresPerHour.push(monthHours[month][day][hour]['totalfares'])
            totalMilesPerHour.push(monthHours[month][day][hour]['totalmiles'])
          })
          this.setState({
            showMiniTable2: true,
            day: day,
